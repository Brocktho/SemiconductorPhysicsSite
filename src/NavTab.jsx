...
function NavTab({key, id, link, toggle, set, pages=[]}) {
    ...
    && pages.map((item,i) => {
        ...
        return(
            <Link key={item.key} id={item.key} to={"/SemiconductorPhysicsSite/" + item.path} className={"w-11/12 text-xs font-bold border-black h-6 pt-1 items-center text-center active:underline" + finalClass}>{item.name}</Link>
        )
    });
